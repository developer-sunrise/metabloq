...
import { CollectionsData,CollectionsDataDynamic } from "../../components/collections/CollectionsData";
...
function VirtualCollection(props) {
  ...
  return (
    <Stack gap={3}>
      <Row>
      {allCollection.filter(item => item.collection_category == "Virtual Real Estate").slice(0, next)?.map((item,i) => {
            return (
