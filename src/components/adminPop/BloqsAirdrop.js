...
function XdcAirdrop() {
    ...
    const { address, XDC_AirDrop, web3, xdcToken, Token } = reduxItems;
    ...
    const [AvailableMetabloqs, setAvailableMetabloqs] = useState(0)
    ...
    const pressingSubmit = async () => {
        ...
        if (!address) {
            ...
            handleClick("warning", "Upload bloqsairdrop excel file");
            return;
        } else {
            let url = "createBloqsAirdrop";
            let params = {
                campaign_name: airdropDetails.campaign_name,
                wallet: address,
                Bloqs_per_wallet: total_xdc,
                Bloqsairdrop_json: airdropDetails.xdcairdrop_json,
            };
    ...
    const getXdcAirdrops = async () => {
        if (address != "") {
            var AvailableXDC = await XDC_AirDrop.methods.AvailableMetabloqs().call();
            console.log("AvailableXDC",AvailableXDC)
            AvailableXDC =web3.utils.fromWei(AvailableXDC,'ether')
            console.log("AvailableXDC",AvailableXDC)
            setAvailableMetabloqs(AvailableXDC)
            let url = "getBloqsAirdrops";
            let params = {
                wallet: address,
            };
            let authtoken = "";
    ...
    const statusChange = async (item, status) => {
        let url = "BloqsAirdropStatus";
        let params = {
            bloqsairdrop_id: item,
            status: status
        };
        let authtoken = "";
        let response = await postMethod({ url, params, authtoken });
    ...
    const depositBloqs = async () => {
        // if(total_xdc){
        const Amount = "1000000000000000000"
        setWalletOpen(true)
        setLoading1(true)
        //   web3.utils.toWei(String(total_xdc),"ether");
        const allowance =await Token.methods.allowance(address, process.env.REACT_APP_XDCAIRDROP_CONTRACT).call()
        console.log("allowance", allowance)
        try {
            if(Number(allowance)<1000000000000000000){
            const Approve =await Token.methods.approve(process.env.REACT_APP_XDCAIRDROP_CONTRACT,'10000000000000000000000').send({ from: address })
            console.log("Approve",Approve)
            }
            try {
                setLoading2(true)
                // setWalletOpen(false)
                var depositbloqs = await XDC_AirDrop.methods.deposit(Amount, false).send({ from: address });
                console.log("desposit", depositbloqs)
                setLoading2(false)
                setLoading1(false)
                setWalletOpen(false)
            } catch (err) {
        ...
        <>
            ...
            <Stack gap={5}>
                <div className="createitempage_createitem">
                    <h2 className="bold poppins">BLOQS AirDrop</h2>
                </div>
                <div className="metabloq_container">
                <Row>
          <Col  className="light-text text-right">
             BLOQS Balance - {AvailableMetabloqs}
            </Col>
          </Row>
                    <Row>
                            ...
                            campaigns.map((data) => (
                                <>
                                    <Col className="fw-bold text-center">
                                        {data.bloqsairdrop_campaign_name}
                                    </Col>
                                    {/* <Col xxl={2} xl={2} lg={2} md={2} className="fw-bold">
                    {Slicer(data.xdcairdrop_wallet)}
                  </Col> */}
                                    <Col className="fw-bold text-center">
                                        {data.bloqsairdrop_no_of_wallet}
                                    </Col>
                                    <Col className="fw-bold text-center">
                                        {formatter.format(data.bloqsairdrop_total_bloqs)}
                                    </Col>
                                    <Col className="fw-bold text-center">
                                        {FormatDate1(data.bloqsairdrop_createdat)}
                                    </Col>
                                    <Col className="d-flex justify-content-center">
                                        {data.bloqsairdrop_status ? (
                                            <button
                                                onClick={() => {
                                                    statusChange(data.bloqsairdrop_id, false);
                                                }}
                                            ...
                                            <button
                                                onClick={() => {
                                                    statusChange(data.bloqsairdrop_id, true);
                                                }}
                            ...
                            :
                            <Row>
                                <Col className="d-flex justify-content-center"> No Data</Col>
                            </Row>
                        }
                    </Row>
                </div>
            ...
            <ActionWallet walletOpen={walletOpen} loader1={Loading1} loader2={Loading2}  hashValue={hashValue} setWalletOpen={setWalletOpen} />
        </>
    );
}
